...
matrix = []
with open('euler81.txt','r') as data:
...
while nextaddy != None:
    ...
    y_ind = nextaddy[1]
    try:
        if grid[x_ind][y_ind].distance + grid[x_ind][y_ind+1].value < grid[x_ind][y_ind+1].distance:
            grid[x_ind][y_ind+1].distance = grid[x_ind][y_ind].distance + grid[x_ind][y_ind+1].value
            grid[x_ind][y_ind+1].previous = (x_ind,y_ind)
    except IndexError:
        temp=1
    try:                
        if grid[x_ind][y_ind].distance + grid[x_ind+1][y_ind].value < grid[x_ind+1][y_ind].distance:
